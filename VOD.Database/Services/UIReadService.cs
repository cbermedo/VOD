...
namespace VOD.Database.Services
{
    public class UIReadService : IUIReadService
    {
        ...
        public async Task<Course> GetCourse(string userId, int courseId)
        {
            ...
            var userCourse = await _db.SingleAsync<UserCourse>(x => x.UserId.Equals(userId)
        ...
        {
            _db.Include<Course>();
            var userCourses = await _db.GetAsync<UserCourse>(x => x.UserId.Equals(userId));
            return userCourses.Select(s => s.Course);
        }
